...
class MovieViewModel extends ViewModel
{
    ...
    public function movie(){
        ...
        $formattedMovie = collect(array_replace($this->movie, [
            ...
            'release_year' => Carbon::parse($this->movie['release_date'])->format('M d, Y'),
            'release_date' => Carbon::parse($this->movie['release_date'])->format('M d, Y'),
            'genres' => $genresFormatted,
            'youtubeURL' => $this->movie['videos']['results'][0]['key'],
        ]))->only([
            'id', 'poster_path', 'title', 'vote_average', 'overview', 'release_date', 'genres', 'genre_ids', 'youtubeURL', 'credits', 'images'
        ]);
