...
class MovieApiService
{
    ...
    public function fetchPopularMovies($max_page = 1)
    {
        ...
        if( !Redis::exists('movies:popular') ){
            ...
            for ($page=1; $page <= $max_page; $page++) { 
                ...
                // Check response OK
                $popularMovies = $this->checkResponseOK($response, 200);
            }
     ...
     * Check response OK
     */
    private function checkResponseOK( array $movie_array, $response, int $statusCode ){

        if($response->getStatusCode() === $statusCode){
            $data = $response->json()['results'];
            return array_merge($movie_array, $data);
        }else{
            abort($response->getStatusCode());
        }
    }   
