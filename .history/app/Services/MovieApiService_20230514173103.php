...
class MovieApiService
{
    ...
    public function fetchRegionMovies(string $region, $page=1, array $filters=null)
    {
        ...
        {
            $url = ($filters == null)
                ? "https://api.themoviedb.org/3/discover/movie?language=en-US&region=".$regionCode."&include_adult=false&include_video=false&page=".$page."&release_date.lte=".$nowDate."&with_original_language=".$regionLanguage 
                : "https://api.themoviedb.org/3/discover/movie?language=en-US&region=".$regionCode."&include_adult=false&include_video=false&page=".$page."&primary_release_year=".$filters['year']."&with_original_language=".$regionLanguage ;

            $movies = $this->fetch($url, $page, 'results');
