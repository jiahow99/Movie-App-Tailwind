...
class MovieApiService
{
    ...
    public function fetchPopularMovies($max_page = 1)
    {
        ...
        if( !Redis::exists('movies:popular') ){
            ...
            for ($page=1; $page <= $max_page; $page++) { 
                ...
                $movies = $this->fetch('https://api.themoviedb.org/3/movie/popular', $page, 'results');
                
                // Check response OK
                $popularMovies = array_merge($popularMovies, $movies);
            }
    ...
    public function fetchNowPlaying($max_page = 1)
    {
        if( !Redis::exists("movies:nowPlaying") ){
            ...
            for ($page=1; $page <= $max_page; $page++) { 
                ...
                $nowPlayingMovies = $this->fetch('https://api.themoviedb.org/3/movie/now_playing', $page, 'results');
    
                // Check response OK
                $nowPlaying = array_merge($nowPlaying, $nowPlayingMovies);
            }
    ...
    {
        return array_merge($movies_array, $response);
    }   
