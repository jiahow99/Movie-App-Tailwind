...
class ActorController extends Controller
{
    ...
    {
        // Actor details (with social media acc)
        $response_actor = Http::withToken(config('services.tmdb.token'))
        ->get('https://api.themoviedb.org/3/person/' . $id . '?append_to_response=external_ids');
        
        if($response_actor->getStatusCode() === 200){
            $actor = $response_actor->json();
        }else{
            abort($response_actor->getStatusCode());
        }
        ...
        if($response_credits->getStatusCode() === 200){
            $credits = $response_credits->json()['cast'];
        }else{
            abort($response_actor->getStatusCode());
        }
