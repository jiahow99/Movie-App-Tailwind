...
class MovieController extends Controller
{
     ...
     */
    private function rate(string $action, string $movie_id, int $score)
    {
        // Rating
        $data = ['value' => $score];
        ...
        $request = Http::withToken(config('services.tmdb.token'))
            ->post('https://api.themoviedb.org/3/movie/'.$movie_id.'/rating', $data);
        ...
        if( $request->json()['success'] )
        {
            ...
            Redis::hset('movies', $movie_id, $json_encoded, 'EX', 1800);  // Expire in 30 mins
