            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap:0 xl:gap-10">
                ...
                @foreach (array_reverse($movie['images']['backdrops']) as $index=>$image)
                    @if ($loop->index < 6)
                        ...
                        mySwiper.navigation.enabled = true;
                        ">
                            <img src="https://image.tmdb.org/t/p/w500/{{ $image['file_path'] }}" alt="{{ $movie['title'] }}_thumbnail" data-index="{{ $index }}" x-on:click="mySwiper.navigation.enabled = true">
                        </div>
                    @endif
                @endforeach
