                ...
                <div class="flex justify-start align-middle gap-5 mt-2">
                    ...
                    @foreach ($filterData['regions'] as $key => $item)
                        <a href="{{ route('movies.region', $key) }}">
                            <div class="w-28 text-center py-1 text-sm duration-200 rounded-full @if( $category == $key ) bg-gray-800 cursor-default @else bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 @endif ">
                                {{ $key }}
                            </div>
                        </a>
                    @endforeach
