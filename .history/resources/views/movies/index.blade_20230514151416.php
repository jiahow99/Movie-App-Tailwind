...
@section('content')
    <div class="container mx-auto px-4 pt-16">
        <!-- Filters -->
        <div class="w-full mb-11 py-3 px-5 bg-gray-600 rounded-md">
            <div class="font-bold">Countries :</div>
            <div class="flex flex-wrap justify-start align-middle gap-5 mt-2">
                <div class="px-4 py-1 duration-300 hover:-translate-y-1 rounded-full underline pointer-events-none bg-gray-800">
                    All
                </div>
                @foreach ($regions as $key => $item)
                    <a href="{{ route('movies.region', $key) }}">
                        <div class="w-28 text-center py-1 text-sm duration-200 bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 rounded-full cursor-pointer">
                            {{ $key }}
                        </div>
                    </a>
                @endforeach
