                ...
                <div class="flex flex-wrap justify-start align-middle gap-5 mt-2">
                    @foreach ($filterData['years'] as $year)
                        <a href="{{ route('movies.region', ['region'=> $category, 'year' => $year]) }}">
                            <div class="w-20 text-center py-1 text-sm duration-200 rounded-full @if( $year == $chosenYear ) bg-gray-800 font-bold cursor-default shadow-lg @else bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 text-gray-400 @endif">
                                {{ $year }}
                            </div>
                        </a>
                    @endforeach
