            ...
            <div class="flex justify-start align-middle gap-5 mt-2">
                ...
                @foreach ($filterData as $key => $item)
                    <a href="{{ route('movies.region', $key) }}">
                        <div class="px-3 py-1 duration-200 rounded-full @if( $category == $key ) bg-gray-500 hover:-translate-y-1 hover:bg-gray-800 cursor-pointer @else bg-gray-800 underline pointer-events-none @endif ">
                            {{ $key }}
                        </div>
                    </a>
                @endforeach
