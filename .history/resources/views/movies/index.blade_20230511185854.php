...
<div class="container mx-auto px-4 pt-8">
    <div class="movies-filter w-full bg-gray-800 px-3 py-5 rounded-lg flex justify-start align-middle gap-5">
        <div class="rounded-sm text-sm px-2 py-1 font-semibold">Countries :</div>
        <div class="rounded-xl px-5 py-2 text-gray-300 cursor-pointer underline-offset-3 hover:bg-slate-700 duration-300 bg-gray-700 underline pointer-events-none">All</div>
        @foreach ($regions as $key => $value)
        <a href="#">
            <div class="rounded-xl text-sm font-semibold bg-slate-500 px-5 py-1 text-gray-300 underline-offset-3 cursor-pointer hover:bg-slate-700 duration-300 select-none">{{ $key }}</div>
        </a>
        @endforeach
    </div>
