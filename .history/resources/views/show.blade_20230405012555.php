...
<div x-data="{
    ...
    x-data="{ imageModalOpen: true, imageSrc: '' }">
        <div class="container mx-auto px-4 py-16">
        ...
        <div class="fixed inset-0 z-20 bg-black/70" x-show="imageModalOpen" x-transition>
            ...
            <div class="flex justify-center align-middle w-8/12 mx-auto mt-36">
                <div class="my-auto mx-5"><i class="fa-solid fa-caret-left text-gray-800 text-4xl cursor-pointer rounded-full bg-white px-2 hover:text-white hover:bg-slate-700 duration-150 thumbnail-prev"></i></div>
                <div class="swiper mySwiper" x-show="imageModalOpen" style="display: none">
                    <div class="swiper-wrapper">
                    @foreach (array_reverse($movie['images']['backdrops']) as $image)
                        <div class="swiper-slide"><img src="https://image.tmdb.org/t/p/original/{{ $image['file_path'] }}"  alt=""></div>
                    @endforeach
                    </div>
                </div>
                <div class="my-auto mx-5"><i class="fa-solid fa-caret-right text-gray-800 text-4xl cursor-pointer rounded-full bg-white px-2 hover:text-white hover:bg-slate-700 duration-150 thumbnail-next"></i></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    var swiper = new Swiper(".mySwiper", {
        ...
        noSwipingSelector: '.thumbnail-next',
    });
</script>
