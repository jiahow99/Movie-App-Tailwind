...
<div x-data="{
    ...
    x-data="{ imageModalOpen: false, imageSrc: '' }">
        ...
        <div class="fixed inset-0 z-20 bg-black/70" x-show="imageModalOpen" x-transition>
            ...
            <div class="flex justify-center align-middle w-12/12 md:w-11/12 xl:w-8/12 mx-auto mt-72 md:mt-56 xl:mt-36">
                <div class="my-auto mx-auto w-1/5"><i class="fa-solid fa-caret-left text-gray-800 text-xl md:text-2xl xl:text-4xl cursor-pointer rounded-full bg-white px-2 hover:text-white hover:bg-slate-700 duration-150 thumbnail-prev no-swiping"></i></div>
                <div class="swiper mySwiper" x-show="imageModalOpen" style="display: none">
                    <div class="swiper-wrapper">
                    @foreach (array_reverse($movie['images']['backdrops']) as $index=>$image)
                        <div class="swiper-slide"><img src="https://image.tmdb.org/t/p/original/{{ $image['file_path'] }}"></div>
                    @endforeach
                    </div>
                </div>
                <div class="my-auto mx-auto w-1/5"><i class="fa-solid fa-caret-right text-gray-800 text-xl md:text-2xl xl:text-4xl cursor-pointer rounded-full bg-white px-2 hover:text-white hover:bg-slate-700 duration-150 thumbnail-next no-swiping"></i></div>
            </div>
        </div>
    </div>
</div>
