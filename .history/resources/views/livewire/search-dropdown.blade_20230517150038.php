                ...
                @if ($searchResults->isNotEmpty())
                    @foreach ($searchResults as $movie)
                        <div class="movie w-full bg-gray-800/70 hover:bg-gray-900 transition-all duration-300 mt-5 px-10 py-4 flex justify-between items-center">
                            ...
                            <div class="right p-3 border-2 border-yellow-400 rounded-full">
                                <span class="text-xl">{{ $movie['vote_average'] }}/10</span>
                            </div>
                        </div>
                    @endforeach
                @else
                    <div class="flex justify-center text-2xl text-gray-300 font-bold mt-32 bg-gray-800 py-2 px-4 w-fit rounded-full">No Results Found</div>
                @endif   
