<div class="w-full mb-8 py-3 px-5 bg-gray-600/50 rounded-md">
    ...
    <div id="filter-menu" class="hidden lg:block">
        ...
        @isset( $filterData['years'] )
            ...
            <div class="flex flex-wrap justify-start align-middle gap-5 mt-2">
                @foreach ($filterData['years'] as $year)
                    <a href="{{ route('movies.region', ['region'=> $category, 'year' => $chosen['year'] ?? $year]) }}">
                        <div class="w-20 text-center py-1 text-sm duration-200 rounded-full @if( $year == $chosen['year'] ) bg-gradient-to-r from-yellow-600 to-red-600 font-bold cursor-default shadow-lg @else bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 text-slate-300 hover:text-white @endif">
                            {{ $year }}
                        </div>
                    </a>
                @endforeach
        ...
        <!-- Years -->
        @isset( $filterData['genres'] )
            <div class="font-bold mt-3">Genres :</div>
            <div class="flex flex-wrap justify-start align-middle gap-5 mt-2">
                @foreach ($filterData['genres'] as $genre)
                    <a href="{{ route('movies.region', ['region'=> $category, 'year' => $chosen['year'] ?? $year, 'genre' => $chosen['genre'] ?? $genre['id'] ]) }}">
                        <div class="w-32 text-center py-1 text-sm duration-200 rounded-full @if( $genre['id'] == $chosen['genre'] ) bg-gradient-to-r from-yellow-600 to-red-600 font-bold cursor-default shadow-lg @else bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 text-slate-300 hover:text-white @endif">
                            {{ $genre['name'] }}
                        </div>
                    </a>
                @endforeach
