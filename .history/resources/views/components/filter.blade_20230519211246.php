            ...
            <div class="flex flex-wrap justify-start align-middle gap-5 mt-2">
                @foreach ($filterData['genres'] as $genre)
                    <a href="{{ route('movies.region', ['region'=> $category, 'year' => $chosen['year'] ?? $year, 'genre' => $chosen['genre'] ?? $genre['id'] ]) }}">
                        <div class="w-32 text-center py-1 text-sm duration-200 rounded-full @if( $genre == $chosen['genre'] ) bg-gradient-to-r from-yellow-600 to-red-600 font-bold cursor-default shadow-lg @else bg-transparent border-2 border-gray-300 hover:-translate-y-1 hover:bg-gray-800 text-slate-300 hover:text-white @endif">
                            {{ $genre['name'] }}
                        </div>
                    </a>
                @endforeach
